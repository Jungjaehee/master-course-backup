...
class LoadDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.train:  # Train set
            ...
            target_wav = np.concatenate([target_wav, np.zeros(self.hp.train.max_audio_len - target_wav.shape[0])], axis=0)

            mixed_padding = np.concatenate([mixed_wav, np.zeros(self.hp.train.max_audio_len - mixed_wav.shape[0])], axis=0)
