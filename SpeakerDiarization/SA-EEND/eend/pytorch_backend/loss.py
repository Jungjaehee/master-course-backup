...
def pit_loss(pred, label, label_delay=0):
    ...
    losses = torch.stack(
        [F.binary_cross_entropy_with_logits(
            pred[label_delay:, ...],
            label[:len(label) - label_delay, ...])])
    
    # return min_loss, label_perms[min_index]
    return losses, label
    '''
    # print("label : ", label_perms[min_index])
    # print(label_perms(min_index).shape)
    return min_loss, label_perms[min_index]


def speaker_loss(pred, label, label_delay=0):
    # [Batch, n_spk, e_dim]

    # losses = torch.stack([F.binary_cross_entropy_with_logits(pred[label_delay:, ...], label[:len(label)-label_delay, ...])])
    # print("embedding : ", pred[0, ...].shape)
    
    losses = torch.stack([1+nn.functional.cosine_similarity(pred[i, ...], pred[i+1, ...], dim=0) for i in range(0, pred.shape[1]-1)])
    return losses, label


def speaker_loss_spk2(pred, label, label_delay=0):

    # losses = torch.stack([F.binary_cross_entropy_with_logits(pred[label_delay:, ...], label[:len(label)-label_delay, ...])])
    # print("embedding : ", pred[0, ...].shape)

    x1 = pred[0, ...].norm(p=2, dim=0, keepdim=True) + 1e-5
    x2 = pred[1, ...].norm(p=2, dim=0, keepdim=True) + 1e-5
    losses = torch.stack([1+nn.functional.cosine_similarity(pred[0, ...]/x1, pred[1, ...]/x2, dim=0)])
    return losses, label
...
def batch_speaker_loss(ys, ts, label_delay=0):
    loss_w_labels = [speaker_loss_spk2(y, t, label_delay) for (y, t) in zip(ys, ts)]
    losses, labels = zip(*loss_w_labels)
    loss = torch.stack(losses).sum()
    n_frames = np.sum([t.shape[0] for t in ts])
    loss = loss / n_frames
